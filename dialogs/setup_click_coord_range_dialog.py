from PyQt5.QtWidgets import QDialog, QVBoxLayout, QLabel, QLineEdit, QPushButton, QMessageBox
from PyQt5.QtGui import QIntValidator

class SetupClickCoordRangeDialog(QDialog):
    """Dialog for setting a coordinate offset range to apply randomness to existing click positions.

    Attributes:
        result (int): The range value entered by the user. Used to apply range offset to existing coordinates.
    """

    def __init__(self, parent=None) -> None:
        """Initialize dialog with a single integer input field for coordinate range."""
        super().__init__(parent)
        self.setWindowTitle("Setup Click Coordinates")
        self.layout = QVBoxLayout()

        # Input for the randomness range (will be added/subtracted from original coords)
        self.x_input = QLineEdit(self)
        int_validator = QIntValidator(self)
        self.x_input.setValidator(int_validator)

        self.layout.addWidget(QLabel("Range:"))
        self.layout.addWidget(self.x_input)

        self.ok_button = QPushButton("OK")
        self.ok_button.clicked.connect(self.accept)
        self.layout.addWidget(self.ok_button)

        self.setLayout(self.layout)
        self.result = None

    def accept(self) -> None:
        """Validate and save the integer range value to be used as offset."""
        if not self.x_input.text():
            QMessageBox.warning(self, "Warning", "All fields must be filled out.")
            return
        try:
            x = int(self.x_input.text())
            self.result = (x)
        except ValueError:
            QMessageBox.warning(self, "Warning", "Invalid input.")
            return
        super().accept()