...
class SetupCoordRangeDialog(QDialog):
    """Dialog for defining a coordinate range (min/max for X and Y) used in move/move-click actions.

    Attributes:
        result (tuple[int, int, int, int]): A tuple containing (min_x, max_x, min_y, max_y)
    """

    def __init__(self, parent=None) -> None:
        """Initialize the dialog layout and input fields for min/max X and Y coordinates."""
    ...
    def accept(self) -> None:
        """Validate inputs and set result as a tuple of 4 integers (min_x, max_x, min_y, max_y)."""
